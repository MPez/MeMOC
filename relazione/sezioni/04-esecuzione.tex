...
\begin{description}
	...
	\item[\texttt{-z arg}]: modalità \modo{istanze esistenti}, parte dal presupposto che le istanze sono già state generate e si occupa di risolverle mediante i programmi \script{cplex} e \script{tabusearch}, elabora i risultati e genera i grafici; l'argomento \texttt{arg} serve per indicare il numero di problemi da risolvere;
\end{description}
...
Una volta aperto l'archivio contenente il codice sorgente ed essersi assicurati che siano installati e presenti nella variabile di sistema \comando{path} le librerie e i software descritti nella sezione \ref{sec:ambiente} è necessario procedere alla compilazione del codice \acronimo{c$++$}, prima di eseguire il programma tramite script \emph{bash}: per fare questo è sufficiente digitare ed eseguire il comando \script{make} all'interno della cartella radice del progetto, per compilare \script{cplex}, ed eseguire nuovamente il comando \script{make} all'interno della cartella \script{eserc.lab.02} per compilare \script{tabusearch}.
